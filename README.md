# 20 Days of Russian — course site

Starter repo for a 20-day Russian course, built with the same pipeline as
...
│   ├── Free Preview/
│   │   ├── Day 01.md              # full free days, published on the public site
│   │   └── Day 02.md
│   └── assets/audio/              # audio files for the free preview days
├── full-course-source/            # PRIVATE — Days 3-20 live here, gitignored
│   ├── Day 03.md ... Day 20.md    # not published; not pushed to GitHub
│   └── assets/audio/
...
the public site — it's just where you keep working on Days 3-20 in Obsidian.
...
1. Open the `russian-course` folder as an Obsidian vault (or open `docs/` and
   ...
   fully apart).
2. Write/edit a day's note: paste the text, drop the audio file into the
...
4. Days in `full-course-source/` never need to be pushed — just keep them
   written and ready for when you package the full course.

## Video (optional, per day)

Each day's note now has a **Video** section (below Audio) for students who
...
Leave the Video section out entirely on days where you don't have footage —
it's optional, not required for every day.

## Selling the full course

Simplest path (Option A from your plan with Claude): once all 20 days are
...
download, that's Option B — deploying the full 20-day site to Cloudflare
Pages with Cloudflare Access gating everything past Day 2. Ask Claude to set
that up when you're ready; it reuses this same `docs/` structure, just with
all 20 days included and access rules layered on top.
